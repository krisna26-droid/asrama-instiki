<x-layouts.penghuni 
    ...
    @if(session('error'))
        <div class="p-4 bg-rose-100 border border-rose-300 text-rose-800 rounded-xl text-xs font-medium">
            {{ session('error') }}
        </div>
    @endif

    @if($reservasiAktif)
        <!-- Kartu Tampilan Jika Sudah Punya Pengajuan/Kamar (Form Otomatis Dikunci) -->
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <div>
                    ...
                    <h3 class="text-lg font-bold text-slate-900 mt-0.5">Status Pengajuan Kamar Anda</h3>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                <div class="p-3 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <span class="font-bold text-slate-800 text-sm">
                        {{ $reservasiAktif->kamar ? 'Kamar ' . $reservasiAktif->kamar->nomor_kamar . ' (Blok ' . $reservasiAktif->kamar->blok . ')' : '-' }}
                    </span>
                </div>
                <div class="p-3 bg-slate-50 rounded-xl border border-slate-100">
    ...
    @else
        <!-- Multi-Step Wizard Card (Hanya muncul jika belum pernah reservasi) -->
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-8">
            ...
            <form action="{{ route('penghuni.reservasi.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div @click="selectKamar({{ json_encode($kamar) }})"
                                ...
                                <div class="flex items-center space-x-3">
                                    <div class="w-12 h-12 rounded-lg bg-slate-100 overflow-hidden shrink-0">
                                        ...
                                        @else
                                            <div class="w-full h-full bg-slate-300 flex items-center justify-center text-slate-500 font-bold text-xs">KMR</div>
                                        @endif
                                    </div>
                                    <div>
                ...
                <div x-show="step === 2" class="space-y-4">
                    <h2 class="text-sm font-bold text-slate-900 border-b border-slate-100 pb-2">Langkah 2: Unggah KTP</h2>
                ...
                <div x-show="step === 3" class="space-y-4">
                    <h2 class="text-sm font-bold text-slate-900 border-b border-slate-100 pb-2">Langkah 3: Unggah KTM</h2>
                    <div class="max-w-md mx-auto space-y-3 text-center">
                        <div class="border-2 border-dashed border-slate-300 rounded-xl p-6 bg-slate-50 relative flex flex-col items-center justify-center">
                ...
                <div x-show="step === 4" class="space-y-4">
                    <h2 class="text-sm font-bold text-slate-900 border-b border-slate-100 pb-2">Langkah 4: Tinjau Pengajuan</h2>
